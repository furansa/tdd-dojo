...
TEST_F(SoundexEncoding, CombinesDuplicatedCodesWhen2ndLetterDuplicates1st) {
    // Act
    auto encoded = soundex.encode("Bbcd");

    // Assert
    ASSERT_THAT(encoded, Eq("B230"));
}

TEST_F(SoundexEncoding, DoesNotCombineDuplicatedEncodingsSeparatedByVowels) {
    // Act
    auto encoded = soundex.encode("Jbob");

    // Assert
    ASSERT_THAT(encoded, Eq("J110"));
}
